...
int Render::RendererInit(JNIEnv *env, int *legacy_drawing, uint8_t *flip) {
    EGLint major, minor;
    EGLint numConfigs;
...

    {
        ...
        EGLImageKHR img;
        AHardwareBuffer *newBuffer = NULL;
        int status;
        ...
        status = AHardwareBuffer_allocate(&d0, &newBuffer);
        if (status != 0 || newBuffer == NULL) {
            loge("Failed to allocate native buffer (%p, error %d)", newBuffer, status);
        ...
        uint32_t *pixels;
        if (AHardwareBuffer_lock(newBuffer, AHARDWAREBUFFER_USAGE_CPU_WRITE_OFTEN |
                                            AHARDWAREBUFFER_USAGE_CPU_READ_OFTEN, -1, NULL,
                                 (void **) &pixels) == 0) {
            pixels[0] = 0xAABBCCDD;
            AHardwareBuffer_unlock(newBuffer, NULL);
        } else {
            loge("Failed to lock native buffer (%p, error %d)", newBuffer, status);
            loge("Forcing legacy drawing");
            *legacy_drawing = 1;
            return 1;
        }

        clientBuffer = eglGetNativeClientBufferANDROID(newBuffer);
        if (!clientBuffer) {
            eglCheckError(__LINE__);
        ...
        if (!(img = eglCreateImageKHR(egl_display, EGL_NO_CONTEXT, EGL_NATIVE_BUFFER_ANDROID,
            ...
            if (eglChooseConfig(egl_display, configAttributes, &checkcfg, 1, &numConfigs) !=
                EGL_TRUE) {
...
void Render::RendererUnsetBuffer(void) {
...
void Render::RendererSetBuffer(JNIEnv *env, AHardwareBuffer *buf) {
    const EGLint imageAttributes[] = {EGL_IMAGE_PRESERVED_KHR, EGL_TRUE, EGL_NONE};
    EGLClientBuffer clientBuffer;
    ...
    if (buffer) {
        ...
        image = clientBuffer ? eglCreateImageKHR(egl_display, EGL_NO_CONTEXT,
    ...
    RendererRedraw(env, flip);

    log("renderer_set_buffer %p %d %d", buffer, desc.width, desc.height);
}

void Render::RendererSetWindow(JNIEnv *env, jobject new_surface, AHardwareBuffer *new_buffer) {
    ...
    if (!g_texture_program) {
        g_texture_program = CreateProgram(vertex_shader, fragment_shader);
        if (!g_texture_program) {
        ...
        g_texture_program_bgra = CreateProgram(vertex_shader, fragment_shader_bgra);
    ...
    if (!new_buffer) {
        ...
        checkGlError();
    } else RendererSetBuffer(env, new_buffer);
}

int Render::RendererShouldRedraw(void) {
    return sfc != EGL_NO_SURFACE && eglGetCurrentContext() != EGL_NO_CONTEXT;
}

int Render::RendererRedraw(JNIEnv *env, uint8_t flip) {
    int err = EGL_SUCCESS;
    ...
    if (eglSwapBuffers(egl_display, sfc) != EGL_TRUE) {
        ...
        if (err == EGL_BAD_NATIVE_WINDOW || err == EGL_BAD_SURFACE) {
            log("We've got %s so window is to be destroyed. "
                ...
                eglErrorLabel(err));
            RendererSetWindow(env, NULL, NULL);
            return 0;
        }
    }
...
void Render::RendererPrintFps(float millis) {
    if (renderedFrames)
...
void Render::RendererUpdateRoot(int w, int h, void *data, uint8_t flip) {
...
void Render::RendererUpdateCursor(int w, int h, int xhot, int yhot, void *data) {
...
void Render::RendererSetCursorCoordinates(int x, int y) {
    cursor.x = (float) x;
    cursor.y = (float) y;
}

GLuint Render::CreateProgram(const char *p_vertex_source, const char *p_fragment_source) {
...
void Render::Draw(GLuint id, float x0, float y0, float x1, float y1, uint8_t flip) {
    float coords[20] = {
            x0, -y0, 0.f, 0.f, 0.f,
            x1, -y0, 0.f, 1.f, 0.f,
            x0, -y1, 0.f, 0.f, 1.f,
