...
void ServerRenderer::Init(int data_socket) {
    DEBUG_LOG();
    ...
    hwDesc.width = m_ImgWidth;
    hwDesc.height = m_ImgHeight;
    hwDesc.layers = 1;
    hwDesc.rfu0 = 0;
    hwDesc.rfu1 = 0;
    hwDesc.stride = 0;
    ...
    int rtCode = AHardwareBuffer_allocate(&hwDesc, &m_HWbuffer);
    if(rtCode != 0 || !m_HWbuffer){
        LOG_E("Failed to allocate hardware buffer.");
        return;
    }

    AHardwareBuffer_Desc reportDesc;
    AHardwareBuffer_describe(m_HWbuffer, &reportDesc);

    EGLClientBuffer clientBuffer = eglGetNativeClientBufferANDROID(m_HWbuffer);
    ...
    AHardwareBuffer_sendHandleToUnixSocket(m_HWbuffer, data_socket);
}
